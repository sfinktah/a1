...
public class AppViewHelpers {
   ...
   public static class Fonts {
      ...
      static public Font fontFromResource(String resourceName, float fontSize) {
         ...
         URL fontURL = UltimateClient.class.getClassLoader().getResource(resourceName);
         if (fontURL == null) {
   ...
   public static class Animation {
      ...
      static public void easeOutCards(Player player, JPanel playerPanel, JLayeredPane jLayeredPane, BlackEngineGuiDecor gameEngine) {
         // move all the (visible) cards to the layered pane, store at level playerIndex + 500
         for (Component c : playerPanel.getComponents()) {
            if (c instanceof PokerCardPanel) {
               Integer level = 500 + gameEngine.getPersonIndex(player);
               if (c.isVisible()) {
         ...
         for (Component c : jLayeredPane.getComponentsInLayer(500 + gameEngine.getPersonIndex(player))) {
            if (c.getParent() != null) {
               ...
               public synchronized void actionPerformed(ActionEvent e) {
                  synchronized (clearing) {
                     ...
                     while (it.hasNext()) {
                        ...
                        if (c instanceof PokerCardPanel && c.isVisible()) {
                           ...
                           // else move up (and a bit left or right) using a simple easing formula
                              ...
                              // fade out
                              ((PokerCardPanel) c).setAlpha(c.getY() / 500f);
                              c.repaint();
                           }
                        }
                     }
      ...
      static public void addMouseListeners(PokerCard card, int playerIndex, PokerCardPanel playingCardPanel, JLabel jStatusLabel, JLayeredPane jLayeredPane) {
   ...
   public static class ContainerTransforms {
       ...
       */
      public static Color getPersonColor(Player player) {
         if (player == null) {
            return Color.BLACK;
         }
         return getPersonColor(StringHelpers.stringAsInt(player.getPersonId(), 0));
      }

      public static Color getPersonColor(int playerIndex) {
         return new Color(
   ...
   public static class Images {
      ...
      public static URL getResourceUrl(String resourceName) {
         return UltimateClient.class.getClassLoader().getResource(resourceName);
      }
      ...
      public static ImageIcon createImageIcon(String path) {
         URL imageUrl = UltimateClient.class.getClassLoader().getResource(path);
         if (imageUrl != null) {
